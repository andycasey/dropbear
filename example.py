...
def parse_name(author_name):
    ...
    is_collaboration = "collaboration" in last_name.lower() \
...
def name_generator(author_name, **kwargs):
    ...
    search_kwds = dict(q=f"author:\"{author_name}\"",
                       rows=rows, start=0, max_pages=max_pages,
    ...
    for j, article in enumerate(ads.SearchQuery(**search_kwds)):
        ...
        for k, (author, aff) in enumerate(zip(article.author, article.aff)):
            ...
            if not k and parsed_author_name["last_name"] == parse_name(author)["last_name"]:
...
def bulk_similarity_search(bibcodes, batch_size=20, rows=20, max_pages=5, **kwargs):
    """
