...
class AuthorNetwork:
    """
    An interface for searching the ADS for the network surrounding an author
    ...
    :param cache_filename: [optional]
        The filename for the cache database. Set to ``None`` for no caching.

    :param cache_ttl: [optional]
        The length of time that cache entries live in the database in seconds.
    """

    def __init__(self, session, cache_filename="cache.db", cache_ttl=86_400):
    ...
    async def network_search(
        ...
        **kwargs,
        ...
        """
        Returns a generator that yields articles found through a network search
        of NASA/ADS, given author names.
        ...
        :param max_initial_rows: [optional]
            The maximum number of initial articles (rows) that will be
            retrieved for *each* entry in `author_names`. The default is 500
            rows (per entry in `author_names`). The total number of articles
            generated will be higher than `len(author_names) *
            max_initial_rows` if a similarity search is also performed.
        ...
        :param similarity_search_on_author_indices: [optional]
            ...
            these authored indices. For example, if `author_names = ("Snow,
            Mary", "Water, M.")` and `similarity_search_on_author_indices = (0,
            3, 5)` then any time an article was found where _Mary Snow_ or _M.
            Water_ appeared as the first, fourth, or sixth author (i.e.,
            zero-indexed), then a similarity search would be performed on that
            article, using the NASA/ADS "similar(bibcode)" functionality.
        ...
        params = dict(
            ...
            + " AND property:refereed",
            fl=fl,
            rows=rows,
            max_pages=max_pages,
        )
        # We await here because we really need this content.
        content = await self._search(start=0, **params)
        ...
        for start in range(rows, max_rows, rows):
            awaitables.append(self._search(start=start, **params))

        # Start streaming back the initial articles.
        for article in content["response"]["docs"]:
            # If the article author matches our similarity author indices, add
            # a coroutine to do a similarity search.
            if (
        ...
        # We use an outer loop here because we might add coroutines to
        # `awaitables` while we are awaiting on those coroutines. In practice
        # we are awaiting on results from future pages from ADS, and we may
        # want to asynchronously do similarity searches on some of those
        # articles that have been returned.
    ...
    async def suggest_authors(
        ...
        **kwargs,
        ...
        """
        Perform a network search of NASA/ADS, given some author name(s), and
        return a generator that will yield suggestions of alternative author
        names, and associated metrics.
        ...
        multiple articles then that author may be suggested multiple times by
        the generator, but successive dictionaries for that author will contain
        updated information about that author (e.g., matched bibcodes,
        ...
        :param affiliation_uniqueness_ratio: [optional]
            The ratio (between 0 and 100) of two affiliation strings in order
            for them to be considered as the same affiliation, based on the
            Levenshtein distance between two affiliation strings. Default is
            75.
    ...
    async def init_cache(self):
        ...
        async with aiosqlite.connect(self.cache_filename) as conn:
            await conn.execute(
                """
    ...
    async def _add_to_cache(self, key, response):
        if self.cache_filename is None:
            return
        ...
        async with aiosqlite.connect(self.cache_filename) as conn:
            await conn.execute(
                """
                INSERT INTO cache (key, created_at, response)
                VALUES (?,?,?)
    ...
    async def _find_in_cache(self, key):
        if self.cache_filename is None:
            return None
        ...
        if row is None or row[1] + self.cache_ttl <= int(time.time()):
    ...
    async def _search(self, retries=3, wait=1, **params):
        ...
        cached = await self._find_in_cache(key)
        ...
        await self._add_to_cache(key, content)
        return content
